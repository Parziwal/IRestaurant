    ...
    public abstract class RestaurantRepositoryTestBase : TestApplicationDbContext
    {
        ...
        public async Task GetRestaurantOverviewList_OnlyThatContainsBudapest_OnOnePage()
        {
            using (var dbContext = CreateDbContext())
            {
                ...
                Assert.Equal(1, restaurantPagedList.PageCount);
                Assert.Equal(TestSeedService.Restaurants[0].Name, restaurantPagedList.Result.ToList()[0].Name);
                Assert.Equal(TestSeedService.Restaurants[1].Name, restaurantPagedList.Result.ToList()[1].Name);
            }
        }
        ...
        public async Task UploadRestaurantImage_ForExistingRestaurant()
        {
            ...
            using (var dbContext = CreateDbContext())
            {
                ...
                var restaurantRepository = CreateRestaurantRepository(dbContext);
